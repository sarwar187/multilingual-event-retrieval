...
def main():
    ...
    result_file = open(os.path.join(data_directory, trg_lang, "results", "data", src_lang, approach,representation, query_type, "output_" + timestr + ".res"), "w")
    ...
    for num_examples in range(1,11):                
        if mode=="search":
            ...
            query_dict = json.load(open(os.path.join(data_directory, src_lang, approach + "_queries", representation, "combined" , str(num_examples) + "_query.json")))                  
            result_string = unsupervised.search_combined_query(query_dict, document_contents, documents_embeddings, document_windows, documents)
